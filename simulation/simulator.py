...
StopMode = Literal["entry_pct", "price_abs", "ref_col"]
...
class StopLossConfig:
    ...
    mode="ref_col":
      Uses a price level from the current row, for example the latest
      market-structure swing low/high column.

      For long trades:
        stop = row[ref_col] * (1 - buffer_pct / 100)

      For short trades:
        stop = row[ref_col] * (1 + buffer_pct / 100)

      If the referenced level is missing, invalid, on the wrong side of entry,
      or too far away by max_ref_risk_pct, the fallback stop is used.
    """
    mode: StopMode = "entry_pct"
    value: float = 0.5

    # Used when mode="ref_col". Example: "5m__ms_4_2__LAST_SWING_LOW".
    ref_col: Optional[str] = None

    # Buffer beyond the referenced structure level.
    # Example: 0.05 means 0.05% below the swing low for longs,
    # or 0.05% above the swing high for shorts.
    buffer_pct: float = 0.0

    # If the structure stop risk is larger than this percentage of entry,
    # use the fallback stop instead.
    max_ref_risk_pct: Optional[float] = None

    # Fallback stop used when ref_col cannot be used.
    # Keep this as "entry_pct" or "price_abs".
    fallback_mode: Literal["entry_pct", "price_abs"] = "entry_pct"
    fallback_value: float = 0.75
...
class TakeProfitConfig:
    ...
    # Used when move_stop_mode="ref_col".
    move_stop_ref_col: Optional[str] = None
    move_stop_buffer_pct: float = 0.0
...
def _calc_stop_price(
    entry_price: float,
    side: Side,
    stop_cfg: Optional[StopLossConfig],
    row: Optional[pd.Series] = None,
) -> Optional[float]:
    """
    Calculates initial stop price.

    Supports fixed stops and market-structure/reference-column stops.

    For mode="ref_col":
      - long:  stop is row[ref_col] minus optional buffer
      - short: stop is row[ref_col] plus optional buffer

    If the referenced stop is invalid or too far from entry, the fallback stop
    is used.
    """
    if stop_cfg is None:
        return None

    def fixed_stop(mode: str, value: float) -> Optional[float]:
        if mode == "entry_pct":
            pct = float(value) / 100.0
            return entry_price * (1.0 - pct) if side == "long" else entry_price * (1.0 + pct)

        if mode == "price_abs":
            return entry_price - float(value) if side == "long" else entry_price + float(value)

        raise ValueError(f"Unsupported fallback stop mode: {mode}. Use 'entry_pct' or 'price_abs'.")

    if stop_cfg.mode in ("entry_pct", "price_abs"):
        return fixed_stop(stop_cfg.mode, stop_cfg.value)

    if stop_cfg.mode == "ref_col":
        if row is None or not stop_cfg.ref_col:
            return fixed_stop(stop_cfg.fallback_mode, stop_cfg.fallback_value)

        try:
            ref = float(row[stop_cfg.ref_col])
        except Exception:
            ref = np.nan
        ...
        buffer = float(stop_cfg.buffer_pct) / 100.0
        ...
        if side == "long":
            ...
            risk_pct = (entry_price - stop) / entry_price * 100.0
            ...
            risk_pct = (stop - entry_price) / entry_price * 100.0

        if stop_cfg.max_ref_risk_pct is not None and risk_pct > float(stop_cfg.max_ref_risk_pct):
            return fixed_stop(stop_cfg.fallback_mode, stop_cfg.fallback_value)
...
def _calc_moved_stop_price(
    ...
    if mode == "ref_col":
        if row is None or not tp_cfg.move_stop_ref_col:
            return None

        try:
            ref = float(row[tp_cfg.move_stop_ref_col])
        except Exception:
            ref = np.nan
        ...
        buffer = float(tp_cfg.move_stop_buffer_pct) / 100.0
        return ref * (1.0 - buffer) if side == "long" else ref * (1.0 + buffer)
...
def run_simulation(
    ...
    def _open_trade(side: Side, reasons: List[str], i: int, t, px: float, equity: float) -> Optional[Trade]:
        ...
        _append_event({
            ...
            "fees": entry_fee,
            "stop_price": tr.stop_price,
            "stop_ref_col": cfg.exit.stop_loss.ref_col if (cfg.exit.enabled and cfg.exit.stop_loss is not None) else None,
            "cash_after": cash,
