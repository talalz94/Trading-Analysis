...
from simulation.rules import RuleGroup, Ctx  # <-- IMPORTANT
...
def align_timeframes(
    ...
    Prefix other timeframe cols with '<tf>__', and base features with '<base_label>__'.
    """
    ...
    for tf, d in other_dfs.items():
        ...
        keep_cols = ["t"] + [
            ...
            if c not in {"open_time","open","high","low","close","volume","quote_volume","num_trades","taker_buy_base","taker_buy_quote"}
            and c != "t"
        ]
        d2 = d2[keep_cols]
...
def run_simulation(
    ...
    stats = {
        ...
        "max_drawdown_pct": float((eq["drawdown"].max() * 100.0) if not eq.empty else 0.0),
